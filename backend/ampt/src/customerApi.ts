...
export const StorefrontApi = api<FastifyPluginAsync>(
  `StorefrontApi`,
  async (fastify) => {
    const store = Store();
    const orders = Orders();
    ...
    fastify.post(
      "/orders",
      useCase<RouteHandlerMethod>(`POST /orders`, async (req, res) => {
        try {
    ...
    fastify.get(
      "/orders/:id",
      useCase<RouteHandlerMethod>(`GET /orders/:id`, async (req, res) => {
        try {
